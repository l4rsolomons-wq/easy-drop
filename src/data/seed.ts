...
import { addDays, mulberry32, pick, randInt, todayISO } from "@/lib/format";
...
export const INSPECTION_ITEMS: { name: string; critical: boolean }[] = [
...
const iso = (date: string, hour: number, minute: number) =>
  `${date}T${String(hour).padStart(2, "0")}:${String(minute).padStart(2, "0")}:00`;

const plate = (rand: () => number) => {
...
const buildRoutes = (): DeliveryRoute[] => [
  {
    id: "RT-001",
...
const buildDrivers = (rand: () => number, today: string): Driver[] => {
  const names = [
  ...
  return names.map((name, i) => {
    ...
    const training = TRAINING_COURSES.map((course, ci) => {
      const roll = rand();
      const status =
        roll > 0.88 ? "Expired" : roll > 0.74 ? "Pending" : ("Completed" as const);
      const date = addDays(today, -randInt(rand, 30, 420));
      return {
        course,
        status: (ci === 0 && i === 6 ? "Expired" : status) as "Completed" | "Pending" | "Expired",
        date,
...
const buildVehicles = (rand: () => number, today: string, routes: DeliveryRoute[]): Vehicle[] => {
  const models = [
    { model: "TVS King Deluxe Cargo", type: "3-Wheel Cargo", fuel: "Petrol" as const, payload: 350 },
    { model: "Bajaj Maxima Cargo", type: "3-Wheel Cargo", fuel: "Petrol" as const, payload: 320 },
    { model: "Piaggio Ape City Cargo", type: "3-Wheel Cargo", fuel: "Petrol" as const, payload: 300 },
    { model: "3Wheel E-Cargo 400", type: "3-Wheel Electric Cargo", fuel: "Electric" as const, payload: 400 },
  ];
  return Array.from({ length: 10 }, (_, i) => {
    ...
    const status: Vehicle["status"] =
      id === "3W-008" ? "Maintenance" : onRoute ? "On Route" : "Available";
...
const statusPlan: [DeliveryStatus, number][] = [
  ["Delivered", 351],
  ["Failed", 5],
  ["Returned", 2],
  ["In Transit", 15],
...
const buildDeliveries = (
  ...
  statuses.forEach((status, i) => {
    ...
    const service: ServiceType =
      serviceRoll < 0.45 ? "Standard Local"
        : serviceRoll < 0.65 ? "Extended Urban"
          : serviceRoll < 0.77 ? "Same-Day Priority"
            : serviceRoll < 0.95 ? "Scheduled Route"
              : "Reverse Logistics";
    const priority: Priority = rand() > 0.88 ? "Urgent" : rand() > 0.68 ? "High" : "Standard";
    const hour = [6, 7, 8, 9, 9, 10, 10, 11, 11, 12, 13, 13, 14, 14, 15, 15, 16, 17][
      Math.floor(rand() * 18)
    ];
    ...
    list.push({
      ...
      priority,
      status,
      hubStage: hubStage as Delivery["hubStage"],
      driverId,
      vehicleId,
...
const buildHistory = (rand: () => number, today: string): DailyStat[] =>
...
const buildMaintenance = (rand: () => number, today: string): MaintenanceJob[] => {
  const services = [
  ...
  return services.map((service, i) => {
    const status: MaintenanceJob["status"] =
      i === 0 ? "In Progress" : i < 3 ? "Due" : i === 3 ? "Overdue" : i < 6 ? "Scheduled" : "Completed";
    return {
      id: `MNT-${2000 + i}`,
...
const buildFuel = (rand: () => number, today: string, vehicles: Vehicle[]): FuelRecord[] => {
...
const buildIncidents = (today: string): Incident[] => [
  {
...
const buildNearMisses = (today: string): NearMiss[] => [
...
const buildTelematics = (rand: () => number, today: string): TelematicsEvent[] => {
  const types: TelematicsEvent["type"][] = ["Harsh Braking", "Harsh Acceleration", "Speeding", "Idling", "Sharp Cornering"];
...
const buildDocuments = (rand: () => number, today: string, vehicles: Vehicle[], drivers: Driver[]): ComplianceDoc[] => {
  const docs: ComplianceDoc[] = [];
  const statusFor = (expires: string): ComplianceDoc["status"] => {
...
const buildInspections = (rand: () => number, today: string, vehicles: Vehicle[]): Inspection[] =>
  vehicles.slice(0, 9).map((v, i) => ({
    ...
    items: INSPECTION_ITEMS.map((it) => ({ name: it.name, critical: it.critical, result: "Pass" as const, notes: "" })),
  }));
...
const buildNotifications = (today: string): AppNotification[] => [
...
const buildActivities = (today: string): Activity[] => [
