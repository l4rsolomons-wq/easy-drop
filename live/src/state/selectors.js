...
export function getRangeStats(state, range) {
    ...
    const slice = range === "yesterday" ? hist.slice(-1) : range === "7d" ? hist.slice(-7) : hist.slice(-30);
    const prev = range === "yesterday" ? hist.slice(-2, -1) : range === "7d" ? hist.slice(-14, -7) : hist.slice(0, 15);
...
export function hourlySeries(state) {
...
export const canDispatch = (state, vehicleId) => {
    ...
    if (v.status === "Maintenance")
        return { ok: false, reason: v.blockedReason ?? "In maintenance" };
    if (v.status === "Out of Service")
        return { ok: false, reason: v.blockedReason ?? "Out of service" };
    return { ok: true };
};
...
export function driverDeliveries(state, driverId) {
    return state.deliveries.filter((d) => d.driverId === driverId);
}
export function kpiRows(state) {
    const t = todayStats(state);
...
export function calcBreakEven(i) {
    ...
    const perVehicleMonthly = i.driverCostPerVehicle + i.fuelPerVehicle + i.maintenancePerVehicle + i.insurancePerVehicle + i.financePerVehicle;
...
export function globalSearch(state, qRaw) {
    const q = qRaw.trim().toLowerCase();
    if (q.length < 2)
        return [];
    const hits = [];
    const push = (h) => hits.length < 40 && hits.push(h);
    state.deliveries.forEach((d) => {
...
export function fuelSummary(state) {
