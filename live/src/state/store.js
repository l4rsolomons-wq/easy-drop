...
import { createContext, useCallback, useContext, useEffect, useMemo, useReducer, useRef, useState, } from "https://esm.sh/react@19.2.6";
import { createSeedState, HUB, SEED_VERSION, SERVICE_PRICE, SUBURBS } from "./../data/seed.js";
import { mulberry32, pick, randInt } from "./../lib/format.js";
...
let idCounter = 0;
const uid = (prefix) => `${prefix}-${Date.now().toString(36).slice(-5)}${(idCounter++).toString(36)}`;
const notify = (state, n) => [
    { id: uid("NTF"), at: nowISO(), read: false, ...n },
    ...state.notifications,
].slice(0, 80);
const logActivity = (state, a) => [{ id: uid("ACT"), at: nowISO(), ...a }, ...state.activities].slice(0, 60);
const addEvent = (d, label, detail, actor) => ({
    ...d,
    events: [...d.events, { at: nowISO(), label, detail, actor }],
});
const etaFromNow = (mins) => new Date(Date.now() + mins * 60000).toISOString();
/* ------------------------------------------------------------------ */
...
function reducer(state, action) {
    switch (action.type) {
        ...
        case "ASSIGN_DELIVERY": {
            ...
            const routes = state.routes.map((r) => r.id === action.routeId && !r.stops.includes(action.id)
                ? { ...r, stops: [...r.stops, action.id] }
                : r);
            return {
                ...state,
                routes,
                deliveries: state.deliveries.map((d) => d.id === action.id
                    ? addEvent({
                        ...d,
                        ...
                        status: "Assigned",
                        hubStage: "Assigned",
                    }, "Assigned", `${driver?.name ?? "Driver"} · ${vehicle?.id ?? "Vehicle"}${action.routeId ? ` · ${action.routeId}` : ""}`, "Dispatch")
                    : d),
                notifications: notify(state, {
                    kind: "driver",
                    tone: "info",
        ...
        case "ADVANCE_DELIVERY": {
            ...
            const deliveries = state.deliveries.map((d) => d.id === action.id
                ? addEvent({
                    ...d,
                ...
                }, action.status, detailMap[action.status] ?? "", driver?.name ?? "Operations")
                : d);
            let vehicles = state.vehicles;
            if (action.status === "Dispatched" && target.vehicleId) {
                vehicles = state.vehicles.map((v) => v.id === target.vehicleId && (v.status === "Available" || v.status === "On Route")
                    ? { ...v, status: "On Route" }
                    : v);
            }
            return {
                ...
                vehicles,
                drivers: delivered
                    ? state.drivers.map((d) => d.id === target.driverId ? { ...d, deliveriesToday: d.deliveriesToday + 1 } : d)
                    : state.drivers,
                ...
                activities: logActivity(state, {
                    kind: "delivery",
                    title: action.status === "Dispatched"
                        ? "Vehicle dispatched"
        ...
        case "CAPTURE_POD": {
            ...
            return {
                ...
                deliveries: state.deliveries.map((d) => d.id === action.id
                    ...
                    }, "Proof of delivery captured", `Signed by ${pod.recipient}`, state.drivers.find((x) => x.id === pod.driverId)?.name ?? "Driver")
                    : d),
                notifications: notify(state, {
                    kind: "delivery",
                    tone: "success",
        ...
        case "FAIL_DELIVERY": {
            ...
            return {
                ...state,
                deliveries: state.deliveries.map((d) => d.id === action.id
                    ? addEvent({ ...d, status: "Failed", failureReason: action.reason, attempts: d.attempts + 1, onTime: false, firstAttempt: false }, "Failed", action.reason, state.drivers.find((x) => x.id === d.driverId)?.name ?? "Driver")
                    : d),
                notifications: notify(state, {
                    kind: "delivery",
                    tone: "warning",
        ...
        case "SET_HUB_STAGE":
            return {
                ...state,
                deliveries: state.deliveries.map((d) => d.id === action.id ? addEvent({ ...d, hubStage: action.stage }, `Hub: ${action.stage}`, "", "Hub") : d),
            };
        case "SUBMIT_INSPECTION": {
            ...
            const vehicles = state.vehicles.map((v) => v.id === action.vehicleId
                ? {
                    ...v,
                    lastInspection: state.today,
                    lastInspectionResult: inspection.result,
                    status: criticalFail ? "Inspection Required" : v.status === "Inspection Required" ? "Available" : v.status,
        ...
        case "SET_VEHICLE_STATUS":
            return {
                ...state,
                vehicles: state.vehicles.map((v) => v.id === action.vehicleId
        ...
        case "UPDATE_MAINTENANCE": {
            ...
            const stillBlocked = state.maintenance.some((m) => m.vehicleId === job.vehicleId && m.id !== job.id && m.status === "In Progress");
            return {
                ...state,
                maintenance: state.maintenance.map((m) => (m.id === action.id ? { ...m, status: action.status } : m)),
                vehicles: state.vehicles.map((v) => {
                    if (v.id !== job.vehicleId)
                        return v;
        ...
        case "REPORT_INCIDENT": {
            ...
            return {
                ...
                drivers: state.drivers.map((d) => d.id === incident.driverId ? { ...d, incidents: d.incidents + 1, safetyScore: Math.max(40, d.safetyScore - 6) } : d),
                notifications: notify(state, {
                    kind: "safety",
        ...
        case "SIMULATE_TICK": {
            ...
            const telematics = rand() > 0.72 && moving.length
                ? [
                    {
                        id: uid("TEL"),
                        vehicleId: pick(rand, moving).id,
                        type: pick(rand, ["Harsh Braking", "Harsh Acceleration", "Speeding", "Sharp Cornering", "Idling"]),
                        time: nowISO(),
        ...
        case "SHIFT": {
            ...
            return {
                ...state,
                drivers: state.drivers.map((d) => d.id === action.driverId
...
export function StoreProvider({ children }) {
    ...
    const value = useMemo(() => ({ state, dispatch, toasts, toast, dismissToast }), [state, dispatch, toasts, toast, dismissToast]);
